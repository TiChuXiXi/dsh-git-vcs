...
面板只渲染一块**居中空态**，只有三样东西：Git 图标 + 「当前目录不是 Git 仓库」+ 目录路径，
下面一个 **「在当前目录初始化 Git 仓库」** 按钮——页签、工具栏、提交树等一律不显示
（那些内容在非仓库下全是空壳），也没有多余的长篇说明。

点按钮先展开**初始分支名**：可自由输入，另有 `main` / `master` 两个一键备选（回车即确认），
点「初始化」才真的执行 `git init`；成功后空态自动消失、面板自己长回完整功能（不必再点 Refresh）。
分支名之所以要用户过一眼：`git config init.defaultBranch` 因机器而异（本机就是 `master`），
替用户决定会得到一个意料之外的分支名。不传 `branch` 时 host 侧的兜底顺序是
入参 → `init.defaultBranch` → `main`。
...
| 脚本 | 用途 |
|------|------|
| `node scripts\verify-host.mjs [仓库]` | 不需要挂 profile，直接在宿主域内跑全部 host 端点（真实 git、临时仓库）。覆盖解析结果、错误码门禁、`allowPush` 门禁、相对路径/非仓库拒绝、未跟踪文件 diff、勾选提交的 pathspec 回归、提交树覆盖所有分支 + `skip` 分页不串页、`repo/init` 全部分支（建仓库 / 幂等 / 指定分支 / 非法分支名 / 嵌套拒绝 / 未出生分支 oid 归一 / `allowWrite` 门禁），最后再从 `lib/client.js` 的字面量**反查**客户端调用的每个端点在 host 侧都存在等 **64 条断言** |
