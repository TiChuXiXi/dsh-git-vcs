window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function GitBody(props) {
      ...
      /**
       * 提交树的派生数据：标签 → 分支配色 → 每行颜色 → 树列宽度。
       *
       * 这些都是 O(提交数) 的计算，而 hover / 右键菜单每变一次就会重渲染整个 Log（2000 行时
       * 每帧重算一遍 BFS 太亏），所以按引用相等缓存一份。刻意不用 useMemo：renderLog 只在 Log
      ...
      function logGraphData() {
        ...
        const data = {
          ...
          tips,
          colorByName,
          colorOf: logColorOf(tips, colorByName, headHash),
          graphWidth: logGraphWidth(tips),
        }
      ...
      function renderLog() {
        ...
        const { headHash, tips, colorByName, colorOf, graphWidth } = logGraphData()
      ...
      return h('div', { style: S.root, ref: rootRef },
        ...
        h('div', { style: S.bar },
          ...
          Btn({
            ...
            onClick: () => { logLoadedRef.current = 0; setApplied(cwd.trim()); closeDetail() },
            children: '打开仓库',
          }),
        ),
        h('div', { style: S.bar },
