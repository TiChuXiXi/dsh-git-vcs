window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const S = {
      ...
      /* 分支名选择：标签 + 输入框 + main/master 备选 + 初始化/取消。空态里唯一的"表单"。 */
      noRepoForm: { display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '8px', marginTop: '2px' },
      noRepoLabel: { fontSize: '11px', color: MUTED },
      noRepoRow: { display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '6px', flexWrap: 'wrap' },
      chipOn: { borderColor: 'rgba(59,130,246,0.75)', background: 'rgba(59,130,246,0.16)', color: 'inherit' },
    ...
    function GitBody(props) {
      ...
      /** 空态里是否已展开分支名选择（点第一下按钮才展开，避免一进来就是表单）。 */
      const [initAsk, setInitAsk] = useState(false)
      ...
      /**
       * 空态里的「初始化」：先展开分支名选择（可自由输入，另有 main / master 备选），
       * 确认后 `git init`，再按正常流程拉一次快照 —— 成功即 setNoRepo(false)，
       * 面板自己从空态长回完整功能（不需要用户再点一次 Refresh）。
       *
       * 分支名必须让用户过一眼：git 全局 `init.defaultBranch` 因机器而异（本机就是 master），
       * 直接替用户决定会得到一个意料之外的分支名。
       */
      async function initRepo() {
        if (initBusy === true) return
        const branch = initBranch.trim()
        if (branch === '') {
          reportError({ code: 'git-vcs/bad-request', message: '请填写初始分支名' })
          return
        }
        setInitBusy(true)
        const result = await rpc('repo/init', { branch })
        if (!alive.current) return
        setInitBusy(false)
        if (!result.ok) {
          reportError(result.error)
      ...
      /**
       * 非 git 仓库时的整页空态：居中的 Git 图标 + 一句说明 + 一个按钮，**没有多余描述**。
       * 点按钮先展开分支名（可输入，备选 main / master），确认后才真的 init。
       * 只保留最上面的路径栏（否则换个目录都换不了），其余功能一律不渲染。
       */
      function renderNoRepo() {
        const blocked = config !== null && config.allowWrite === false
        // 必须用 h(TextInput, …) 生成元素：TextInput 内部有 useState，直接当函数调用
        // 会把它的 hook 记到父组件头上，`initAsk` 一变就是"Rendered more hooks than during
        // the previous render"（React error #310），整棵面板直接渲染不出来。
        const branchInput = h(TextInput, {
          style: { ...S.input, flex: 'none', width: '150px', textAlign: 'center' },
          placeholder: '初始分支名',
          spellCheck: false,
          value: initBranch,
          onChange: (event) => setInitBranch(event.target.value),
          onKeyDown: (event) => { if (event.key === 'Enter') void initRepo() },
        })
        return h('div', { style: S.noRepo },
          ...
          blocked
            ? h('div', { style: S.noRepoTitle }, '插件配置里 allowWrite=false，已禁用初始化')
            : (initAsk === false
              ? Btn({
                kind: 'primary',
                style: S.buttonBig,
                title: '在当前目录执行 git init',
                onClick: () => setInitAsk(true),
                children: '在当前目录初始化 Git 仓库',
              })
              : h('div', { style: S.noRepoForm },
                h('div', { style: S.noRepoLabel }, '初始分支名'),
                h('div', { style: S.noRepoRow },
                  branchInput,
                  ...['main', 'master'].map((name) => h('button', {
                    key: name,
                    style: initBranch === name ? { ...S.chip, ...S.chipOn } : S.chip,
                    onClick: () => setInitBranch(name),
                  }, name)),
                ),
                h('div', { style: S.noRepoRow },
                  Btn({
                    kind: 'primary',
                    disabled: initBusy || applied === '',
                    title: applied === '' ? '还没有选择目录' : '执行 git init 并创建该分支',
                    onClick: () => { void initRepo() },
                    children: initBusy ? Busy({ text: '正在初始化…' }) : '初始化',
                  }),
                  Btn({ disabled: initBusy, onClick: () => setInitAsk(false), children: '取消' }),
                ),
              )),
        )
      }
